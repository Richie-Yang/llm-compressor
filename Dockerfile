...
# CPU/x86 wheels. transformers is deliberately left out of the constraints and pinned
# explicitly below: llmcompressor needs a newer one than NGC ships.
RUN pip list --format=freeze 2>/dev/null \
    ...
    && PIP_CONSTRAINT=/tmp/constraints.txt \
       pip install --no-cache-dir "llmcompressor==${LLMCOMPRESSOR_VERSION}" "transformers==5.14.1" \
